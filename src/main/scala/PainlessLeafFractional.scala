...
/**
  ...
  */
class PainlessLeafFractional extends Verifier {
  ...
  def minimizeCache_(cache: DAG[Literal]): DAG[Literal] =
    val cexs = cache.leafStream.toList
    // ugly
    ...
    val target = math.ceil(fractionToCover * formCache.size) // for checking whether we are done somewhat quickly
